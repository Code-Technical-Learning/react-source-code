...
function getRollupOutputOptions(
  ...
  return {
    ...
    exports: 'auto',
    sourcemap: true,
    sourcemapPathTransform: (relatePath, sourcemapPath) => {
      const replace = '../../../../packages'.replaceAll('/', sep);
      return relatePath.replace(replace, `${process.cwd()}/packages`);
    },
  };
}
  ...
  return [
    {
      ...
      // transform 函数返回 map 为 null 保留之前转换的映射 ，transformed.generateMap() 只生成类型对应行的 sourcemap 映射 mapping
      async transform(code, file) {
        const transformed = flowRemoveTypes(code, {pretty: true});

        const transformedCode = transformed.toString();
        const transformedMap = transformed.generateMap();

        // const res = babelCore.transformSync(transformedCode, {
        //   sourceMaps: true,
        // });
